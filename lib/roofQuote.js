...
// Turns a Solar API response into what the drawer shows. point: the address's coordinates ({ lat, lng }).
// The map pin goes on the building's center from the roof data, or on the address when there isn't one.
export function summarize(b, label, point) {
  ...
  return {
    label,
    center: b.center || { latitude: point.lat, longitude: point.lng },
...
// Price range for a material, from the answers in the drawer (quote: sqft, pitches, stories).
// With more than one pitch picked, the low end uses the easiest pitch and the high end the steepest.
export function priceFor(quote, material) {
  const p = PRICING[material];
  const pitchAdj = (quote.pitches.length ? quote.pitches : ['Not sure']).map((pitch) => PITCH_ADJ[pitch] || 1);
  const base = quote.sqft * WASTE * (STORY_ADJ[quote.stories] || 1);
  return {
    label: p.label,
    low: round100(Math.max(MIN_JOB, base * Math.min(...pitchAdj) * p.low)),
    high: round100(Math.max(MIN_JOB * 1.2, base * Math.max(...pitchAdj) * p.high)),
  };
}
